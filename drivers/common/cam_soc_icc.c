...
#define pr_fmt(fmt) "CAM-ICC %s:%d " fmt, __func__, __LINE__

#ifdef CONFIG_CAM_SOC_ICC_DBG
...
int cam_soc_bus_client_update_request(void *client, unsigned int idx)
{
	...
	if (idx >= bus_client->common_data->num_usecases) {
		pr_err("Invalid vote level=%d, usecases=%d", idx,
...
int cam_soc_bus_client_update_bw(void *client, uint64_t ab, uint64_t ib)
{
	...
	CDBG("Bus client=[%s] :ab[%llu] ib[%llu]",
		bus_client->common_data->name, ab, ib);
	rc = icc_set_bw(bus_client_data->icc_data, Bps_to_icc(ab),
		Bps_to_icc(ib));
	if (rc) {
		pr_err("Update request failed, client[%s]",
...
int cam_soc_bus_client_register(
	struct platform_device *pdev, void **client,
	struct cam_soc_bus_client_common_data *common_data)
{
	...
	*client = bus_client;
	...
	if (IS_ERR_OR_NULL(bus_client_data->icc_data)) {
		pr_err("failed in register bus client");
		rc = -EINVAL;
		goto error;
	}
	...
	if (rc) {
		pr_err("Bus client update request failed, rc = %d",
			rc);
		goto fail_unregister_client;
	}

	CDBG("Register Bus Client=[%s] : src=%d, dst=%d",
