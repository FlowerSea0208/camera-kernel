...
static void cam_icp_ctx_timer_cb(struct timer_list *timer_data)
{
	...
	cam_req_mgr_worker_enqueue_task(task, &icp_hw_mgr,
		CRM_TASK_PRIORITY_0);
	spin_unlock_irqrestore(&icp_hw_mgr.hw_mgr_lock, flags);
}
...
static int32_t cam_icp_hw_mgr_cb(void *data, bool recover)
{
	...
	rc = cam_req_mgr_worker_enqueue_task(task, &icp_hw_mgr,
		CRM_TASK_PRIORITY_0);
	spin_unlock_irqrestore(&hw_mgr->hw_mgr_lock, flags);
...
static int cam_icp_mgr_create_workers(void)
{
	int rc;
	int i;

	rc = cam_req_mgr_worker_create("icp_command_queue", ICP_WORKQ_NUM_TASK,
		&icp_hw_mgr.cmd_work, CRM_WORKER_USAGE_NON_IRQ, CAM_WORKER_FLAG_HIGH_PRIORITY);
	if (rc) {
		CAM_ERR(CAM_ICP, "unable to create a command worker");
		goto cmd_work_failed;
	}

	rc = cam_req_mgr_worker_create("icp_message_queue", ICP_WORKQ_NUM_TASK,
		&icp_hw_mgr.msg_work, CRM_WORKER_USAGE_IRQ, CAM_WORKER_FLAG_HIGH_PRIORITY);
	if (rc) {
		CAM_ERR(CAM_ICP, "unable to create a message worker");
		goto msg_work_failed;
	}

	rc = cam_req_mgr_worker_create("icp_timer_queue", ICP_WORKQ_NUM_TASK,
		&icp_hw_mgr.timer_work, CRM_WORKER_USAGE_IRQ, 0);
	if (rc) {
...
cmd_work_data_failed:
	cam_req_mgr_worker_destroy(&icp_hw_mgr.timer_work);
timer_work_failed:
	cam_req_mgr_worker_destroy(&icp_hw_mgr.msg_work);
msg_work_failed:
	cam_req_mgr_worker_destroy(&icp_hw_mgr.cmd_work);
cmd_work_failed:
	return rc;
}
	...
	int *iommu_hdl, cam_icp_mini_dump_cb mini_dump_cb, bool *node_check)
{
	...
	rc = cam_icp_mgr_create_workers();
	if (rc) {
		CAM_ERR(CAM_ICP, "cam_icp_mgr_create_workers fail: rc=%d", rc);
		goto icp_workers_create_failed;
	}
