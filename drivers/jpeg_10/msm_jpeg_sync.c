	...
	void __user *to)
{
	...
	if (is_compat_task()) {
#ifdef CONFIG_COMPAT
		msm_jpeg_put_ctrl_cmd32(&ctrl_cmd, (void __user *) to);
#endif
	} else {
	...
	void __user *to)
{
	...
	if (is_compat_task()) {
#ifdef CONFIG_COMPAT
		return msm_jpeg_put_jpeg_buf32(&buf_cmd, (void __user *) to);
#endif
	} else {
	...
	void __user *arg)
{
	...
	if (is_compat_task()) {
#ifdef CONFIG_COMPAT
		rc = msm_jpeg_get_jpeg_buf32(&buf_cmd, (void __user *) arg);
		if (rc < 0) {
