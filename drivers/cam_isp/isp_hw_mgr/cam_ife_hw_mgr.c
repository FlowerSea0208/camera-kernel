	...
	struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	if (c_ctx->is_tpg)
		cam_ife_hw_mgr_deinit_hw_res(&c_ctx->res_list_tpg);
	...
	struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	if (c_ctx->is_tpg) {
		CAM_DBG(CAM_ISP, "INIT TPG ... in ctx id:%d",
			c_ctx->ctx_index);
		rc = cam_ife_hw_mgr_init_hw_res(&c_ctx->res_list_tpg);
		if (rc) {
			CAM_ERR(CAM_ISP, "Can not INIT TFE TPG(id :%d)",
				c_ctx->res_list_tpg.hw_res[0]->hw_intf->hw_idx);
			goto deinit;
		}
	}
	...
	struct cam_ife_hw_mgr_ctx  *ife_ctx)
{
	...
	if (c_ctx->is_tpg)
		cam_ife_hw_mgr_free_hw_res(&c_ctx->res_list_tpg);
	...
	enum cam_ife_pix_path_res_id        *path_res_id)
{
	...
	*cid_res = NULL;
	...
	if (c_ctx->is_tpg) {
		if (c_ctx->res_list_tpg.hw_res[0]->hw_intf->hw_idx == 0)
	...
	CAM_DBG(CAM_ISP, "acquire new rsrc fe: %d usage_type %d dsp %d rsrc %p",
		c_ctx->is_fe_enabled, in_port->usage_type,
		c_ctx->dsp_enabled, csid_acquire.node_res);
...
acquire_successful:
	*path_res_id = csid_acquire.res_id;
	...
	if (cid_res_temp->is_dual_isp && *path_res_id
		...
		if (c_ctx->is_tpg) {
			if (c_ctx->res_list_tpg.hw_res[0]->hw_intf->hw_idx
				== 0)
				csid_acquire.phy_sel = CAM_ISP_IFE_IN_RES_PHY_0;
			else
				csid_acquire.phy_sel = CAM_ISP_IFE_IN_RES_PHY_1;
	...
	uint32_t                                 num_inport)
{
	...
	c_ctx->res_list_tpg.res_type = in_port->res_type;
	c_ctx->res_list_tpg.hw_res[0] = tpg_reserve.node_res;
	c_ctx->is_tpg = true;
...
static int cam_ife_hw_mgr_acquire_res_ife_csid_rdi(
	struct cam_ife_hw_mgr_ctx           *ife_ctx,
	struct cam_isp_in_port_generic_info *in_port,
	enum cam_ife_pix_path_res_id  *acq_res_id)
{
	int rc = -EINVAL;
	int i;
	...
	struct cam_isp_in_port_generic_info        *in_port)
{
	...
	ife_bus_rd_res->is_dual_isp = (uint32_t)c_ctx->is_dual;
	...
	uint32_t                                   *acquired_hw_path)
{
	...
	*acquired_hw_id |= cam_convert_hw_idx_to_ife_hw_num(
	...
	ife_src_res->res_type = vfe_acquire.rsrc_type;
	ife_src_res->res_id = vfe_acquire.vfe_in.res_id;
	ife_src_res->is_dual_isp = (uint32_t)c_ctx->is_dual;
	...
	uint32_t *acq_res_id)
{
	...
	if (in_port->rdi_count) {
		...
		rc = cam_ife_hw_mgr_acquire_res_ife_csid_rdi(ife_ctx, in_port, acq_res_id);
		if (rc) {
			CAM_ERR(CAM_ISP,
	...
	enum cam_cdm_cb_status status, uint64_t cookie)
{
	...
	ctx = ((struct cam_ife_hw_mgr_ctx *)
			hw_update_data->isp_mgr_ctx)->concr_ctx;
...
static int cam_ife_mgr_acquire_hw(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	uint32_t                           input_size = 0;
	uint32_t                           acq_hw_id_mask = 0;
	uint32_t                          *out_hw_id_mask;
	...
	ife_mgr_ctx->in_ports = kcalloc(ife_mgr_ctx->num_in_ports,
			sizeof(struct cam_isp_in_port_generic_info),
			GFP_KERNEL);
	...
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		...
		if ((ife_mgr_ctx->in_ports[i].res_type == CAM_ISP_IFE_IN_RES_CPHY_TPG_0) ||
			(ife_mgr_ctx->in_ports[i].res_type == CAM_ISP_IFE_IN_RES_CPHY_TPG_1) ||
			(ife_mgr_ctx->in_ports[i].res_type == CAM_ISP_IFE_IN_RES_CPHY_TPG_2))
			rc  = cam_ife_hw_mgr_acquire_tpg(ife_mgr_ctx, &ife_mgr_ctx->in_ports[i],
	...
	if (acquire_hw_info->common_info_version > 0x1000) {
		out_hw_id_mask =
			(uint32_t *)((uint8_t *)&acquire_hw_info->data +
			acquire_hw_info->common_info_offset);
		*out_hw_id_mask |= acq_hw_id_mask;
	}
					...
					void *config_hw_args)
{
	...
	if (cfg->reapply && cfg->cdm_reset_before_apply) {
		if (c_ctx->last_cdm_done_req < cfg->request_id) {
			cdm_hang_detect =
				cam_cdm_detect_hang_error(c_ctx->cdm_handle);
			...
			if (rc) {
				CAM_ERR_RATE_LIMIT(CAM_ISP,
					"CDM reset unsuccessful for req: %lld. c_ctx: %d, rc: %d",
	...
	if (cfg->num_hw_update_entries > 0) {
		...
		if (cfg->init_packet ||
			(c_ctx->custom_config & CAM_IFE_CUSTOM_CFG_SW_SYNC_ON)) {
			rem_jiffies = wait_for_completion_timeout(
				&c_ctx->config_done_complete,
				msecs_to_jiffies(60));
			if (rem_jiffies == 0) {
				CAM_ERR(CAM_ISP,
					"config done completion timeout for req_id=%llu c_ctx_index %d",
					cfg->request_id, c_ctx->ctx_index);
				if (cam_cdm_detect_hang_error(c_ctx->cdm_handle))
...
start_only:
	...
	if (ctx->is_tpg) {
		CAM_DBG(CAM_ISP, "START TPG HW ... in ctx id:%d",
			ctx->ctx_index);
		rc = cam_ife_hw_mgr_start_hw_res(&ctx->res_list_tpg, hw_mgr_ctx);
		if (rc) {
		...
		if (i == packet->num_io_configs) {
			*ctx_found = false;
			CAM_ERR(CAM_ISP,
				...
				c_ctx->ctx_index, packet->header.request_id,
				res_id);
			return;
		}
	...
	struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	/* dump the registers  */
	rc = cam_ife_mgr_handle_reg_dump(ctx, c_ctx->reg_dump_buf_desc,
		c_ctx->num_reg_dump_buf,
		CAM_ISP_PACKET_META_REG_DUMP_ON_ERROR, NULL, false);
	if (rc)
		CAM_ERR(CAM_ISP,
			"Reg dump on pf failed req id: %llu rc: %d",
			c_ctx->applied_req_id, rc);
	...
	hw_mgr_res = &c_ctx->res_list_ife_out[res_id_out];
	for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
		if (!hw_mgr_res->hw_res[i])
			continue;
		hw_intf = hw_mgr_res->hw_res[i]->hw_intf;
...
mid_check:
	...
	*resource_type = get_res.out_res_id;
	...
	cam_ife_mgr_pf_dump(get_res.out_res_id, hw_mgr_ctx);

outportlog:
	cam_ife_mgr_print_io_bufs(hw_mgr, *resource_type, packet,
		ctx_found, hw_mgr_ctx);
