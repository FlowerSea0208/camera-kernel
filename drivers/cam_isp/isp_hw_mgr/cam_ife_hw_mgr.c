...
static int cam_ife_mgr_update_core_info_to_cpas(struct cam_ife_hw_mgr_ctx           *hw_mgr_ctx,
	bool set_port)
{
	...
	list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_csid, list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
			...
			rc = cam_ife_mgr_find_core_idx(i, hw_mgr_ctx, CAM_ISP_HW_TYPE_CSID,
				&csid_core_idx);
			if (rc)
				return rc;

			rc = cam_ife_mgr_find_core_idx(i, hw_mgr_ctx, CAM_ISP_HW_TYPE_SFE, &sfe_core_idx);
			if (rc)
				return rc;
...
static int cam_isp_blob_drv_config(struct cam_ife_hw_mgr_ctx         *hw_mgr_ctx,
...
static bool cam_isp_is_ctx_primary_rdi(struct cam_ife_hw_mgr_ctx  *hw_mgr_ctx)
{
	struct cam_ife_hw_concrete_ctx *ctx = hw_mgr_ctx->concr_ctx;
	...
	bool user_triggered_dump)
{
	...
	if (!num_reg_dump_buf || !reg_dump_buf_desc) {
		CAM_DBG(CAM_ISP,
			...
			c_ctx->applied_req_id, c_ctx->ctx_index, meta_type,
		...
		if (reg_dump_buf_desc[i].meta_data == meta_type) {
			...
			if (rc) {
				CAM_ERR(CAM_ISP,
					...
					i, rc, c_ctx->applied_req_id, meta_type);
				return rc;
			}
		}
	}
	...
	bool                            *is_bus_overflow)
{
	...
	for (i = 0; i < c_ctx->num_base; i++) {
		...
		if (c_ctx->base[i].hw_type == CAM_ISP_HW_TYPE_VFE) {
			if (c_ctx->base[i].idx != evt->hw_idx)
				continue;

			hw_if = g_ife_hw_mgr.ife_devices[evt->hw_idx]->hw_intf;
			res_id = ife_res_id;
		} else if (c_ctx->base[i].hw_type == CAM_ISP_HW_TYPE_SFE) {
			if (c_ctx->base[i].idx != evt->in_core_idx)
				continue;
...
err:
	CAM_ERR(CAM_ISP, "RESET HW res failed: (ctx_idx: %u type:%d, id:%d)",
		c_ctx->ctx_index, hw_mgr_res->res_type, hw_mgr_res->res_id);
	return rc;
}
	...
	bool     is_internal_start)
{
	...
	for (j = c_ctx->num_base - 1 ; j >= 0; j--) {
		cnt = 0;
		is_secure = false;

		if (c_ctx->base[j].hw_type != CAM_ISP_HW_TYPE_CSID)
			continue;

		list_for_each_entry(hw_mgr_res, &c_ctx->res_list_ife_csid, list) {
			isp_res = hw_mgr_res->hw_res[c_ctx->base[j].split_id];

			if (!isp_res || c_ctx->base[j].idx != isp_res->hw_intf->hw_idx)
				continue;
		...
		if (cnt) {
			...
			if (c_ctx->cdm_hw_idx < 0) {
				CAM_ERR(CAM_ISP,
					"CSID[%u], physical CDM hw_idx is invalid: %d on ctx: %u",
					hw_intf->hw_idx, c_ctx->cdm_hw_idx, c_ctx->ctx_index);
				return -EINVAL;
			}

			start_args.cdm_hw_idx = c_ctx->cdm_hw_idx;
		...
		if (hw_intf->hw_ops.start) {
			...
			if (rc) {
				CAM_ERR(CAM_ISP,
					...
					hw_intf->hw_idx, c_ctx->ctx_index);
				goto err;
			}
		} else {
			CAM_ERR(CAM_ISP, "function null, ctx_idx: %u", c_ctx->ctx_index);
			goto err;
		}
	}
...
err:
	CAM_ERR(CAM_ISP, "Start hw res failed (ctx_idx: %u type:%d, id:%d)",
		c_ctx->ctx_index, isp_hw_res->res_type, isp_hw_res->res_id);
	return rc;
}
	...
	struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	if (c_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
		/* Deint SFE RSRC */
		list_for_each_entry(hw_mgr_res, &c_ctx->res_list_sfe_src, list) {
			CAM_DBG(CAM_ISP, "Going to DeInit SFE SRC %u, ctx_idx: %u",
				hw_mgr_res->res_id, c_ctx->ctx_index);
			cam_ife_hw_mgr_deinit_hw_res(hw_mgr_res);
		}

		/* Deinit SFE OUT */
		for (i = 0; i < max_sfe_out_res; i++)
			cam_ife_hw_mgr_deinit_hw_res(&c_ctx->res_list_sfe_out[i]);
	}
	...
	for (i = CAM_LLCC_SMALL_1; i < CAM_LLCC_MAX; i++) {
		if (c_ctx->flags.sys_cache_usage[i])
			cam_cpas_deactivate_llcc(i);
		c_ctx->flags.sys_cache_usage[i] = false;
	}

	for (i = 0; i < c_ctx->num_base; i++) {
		if (c_ctx->base[i].hw_type == CAM_ISP_HW_TYPE_SFE)
			for (j = 0; j < CAM_ISP_EXPOSURE_MAX; j++)
				hw_mgr->sfe_cache_info[c_ctx->base[i].idx].activated[j] = false;
	}

	c_ctx->flags.init_done = false;
}
	...
	struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	if (c_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
		...
		for (i = 0; i < max_sfe_out_res; i++) {
			rc = cam_ife_hw_mgr_init_hw_res(
				&c_ctx->res_list_sfe_out[i]);
			if (rc) {
				CAM_ERR(CAM_ISP, "Can not INIT SFE OUT (%d), ctx_idx:%u",
					c_ctx->res_list_sfe_out[i].res_id, c_ctx->ctx_index);
				goto deinit;
			}
		}
	}
	...
	for (i = 0; i < max_ife_out_res; i++) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP, "Can not INIT IFE OUT (%d), ctx_idx:%u",
				c_ctx->res_list_ife_out[i].res_id, c_ctx->ctx_index);
			goto deinit;
		}
	}
	...
	if (hw_mgr->csid_global_reset_en) {
		...
		if (rc) {
			CAM_ERR(CAM_ISP, "CSID reset failed, ctx_idx:%u", c_ctx->ctx_index);
			goto deinit;
		}
	}
	...
	for (i = CAM_LLCC_SMALL_1; i < CAM_LLCC_MAX; i++) {
		if (c_ctx->flags.sys_cache_usage[i]) {
			...
			if (rc) {
				CAM_ERR(CAM_ISP,
				"Failed to activate cache: %d, ctx_idx:%u", i, c_ctx->ctx_index);
				goto deinit;
			}
		}
	}
...
static void cam_ife_hw_mgr_dump_all_ctx(void)
{
	...
	struct cam_isp_hw_mgr_res       *hw_mgr_res;
	struct cam_isp_hw_mgr_res       *hw_mgr_res_temp;
	struct cam_ife_hw_concrete_ctx  *ctx_temp;
	...
	uint32_t num_pd_port, uint32_t num_rdi_port, int acquire_failed)
{
	...
	if (!list_empty(&c_ctx->res_list_sfe_src)) {
		hw_mgr_res = list_first_entry(&c_ctx->res_list_sfe_src,
			struct cam_isp_hw_mgr_res, list);
...
fail:
	CAM_ERR(CAM_ISP,
		...
		(c_ctx->flags.is_dual) ? "dual" : "single",
	...
	struct cam_ife_hw_mgr_ctx    *hwr_mgr_ctx)
{
	...
	CAM_INFO(CAM_ISP,
		...
		hrs, min, sec, ms,
		c_ctx->ctx_index,
		(c_ctx->flags.is_rdi_only_context ? "true" : "false"),
		(c_ctx->flags.is_dual ? "true" : "false"));
	...
	struct cam_ife_hw_mgr_ctx  *ife_ctx)
{
	...
	if (c_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
		...
		list_for_each_entry_safe(hw_mgr_res, hw_mgr_res_temp,
			&c_ctx->res_list_sfe_src, list) {
			cam_ife_hw_mgr_free_hw_res(hw_mgr_res);
			cam_ife_hw_mgr_put_res(&c_ctx->free_res_list,
				&hw_mgr_res);
		}
	}
	...
	enum cam_isp_hw_type             hw_type)
{
	...
	if (!c_ctx->num_base) {
		...
		if (i == c_ctx->num_base) {
			...
			c_ctx->base[c_ctx->num_base].hw_type = hw_type;
			c_ctx->num_base++;
			CAM_DBG(CAM_ISP,
	...
	struct cam_ife_hw_mgr_ctx        *ctx)
{
	...
	if (list_empty(&c_ctx->res_list_ife_src) &&
		list_empty(&c_ctx->res_list_sfe_src)) {
	...
	bool                                 update_only)
{
	...
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		if (!update_only) {
			...
			ife_src_res->num_children++;
			c_ctx->num_acq_vfe_out++;
			CAM_DBG(CAM_ISP, "IFE SRC num_children = %d acq vfe out %d",
				ife_src_res->num_children,c_ctx->num_acq_vfe_out);
		}
	}
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	sfe_acquire.tasklet = c_ctx->common.tasklet_info;

	sfe_out_res = &c_ctx->res_list_sfe_out[sfe_out_res_id & 0xFF];
	for (i = 0; i < in_port->num_out_res; i++) {
		out_port = &in_port->data[i];
		CAM_DBG(CAM_ISP,
		...
		sfe_acquire.sfe_out.cdm_ops = c_ctx->cdm_ops;
		sfe_acquire.sfe_out.out_port_info = out_port;
		sfe_acquire.sfe_out.split_id = CAM_ISP_HW_SPLIT_LEFT;
		sfe_acquire.sfe_out.unique_id = c_ctx->ctx_index;
		sfe_acquire.sfe_out.is_dual = 0;
		sfe_acquire.buf_done_controller = c_ctx->buf_done_controller;

		if (ife_ctx->is_offline) {
			sfe_acquire.priv = c_ctx->hw_mgr;
			sfe_acquire.event_cb =
				cam_ife_hw_mgr_event_handler_offline;
		} else {
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		sfe_out_res = &c_ctx->res_list_sfe_out[k];
	...
	struct cam_sfe_acquire_args        *sfe_acquire)
{
	...
	if ((c_ctx->flags.is_fe_enabled) || (use_lower_idx)) {
		for (i = 0; i < CAM_SFE_HW_NUM_MAX; i++) {
	...
	uint32_t                            *acquired_hw_path)
{
	...
	rc = cam_ife_hw_mgr_get_res(&c_ctx->free_res_list,
		&sfe_src_res);
	if (rc) {
		CAM_ERR(CAM_ISP, "Ctx : %d failed to get res", c_ctx->ctx_index);
		return rc;
	}
	...
	cam_ife_hw_mgr_put_res(&c_ctx->res_list_sfe_src,
		&sfe_src_res);
	...
	uint32_t *acquired_hw_id, uint32_t *acquired_hw_path)
{
	...
	CAM_DBG(CAM_ISP, "ctx: %u Required SFE in resources: 0x%x", c_ctx->ctx_index,
		sfe_required_res);
	...
	CAM_DBG(CAM_ISP, "ctx: %u Acquired SFE in resources: 0x%x", c_ctx->ctx_index,
		sfe_acquired_res);
	return 0;

err:
	CAM_ERR(CAM_ISP, "Acquire SFE failed ctx: %u acquired_res 0x%x sfe_res %u",
		c_ctx->ctx_index, sfe_acquired_res, sfe_res_id);
	...
	struct cam_ife_hw_mgr_ctx                 *ife_ctx)
{
	...
	CAM_DBG(CAM_ISP,
		...
		csid_acquire->in_port->can_use_lite,
		c_ctx->flags.is_rdi_only_context,
	...
	struct cam_isp_in_port_generic_info        *in_port)
{
	...
	vfe_acquire.vfe_bus_rd.is_dual = (uint32_t)c_ctx->flags.is_dual;
	vfe_acquire.vfe_bus_rd.is_offline = c_ctx->flags.is_offline;
	...
	ife_bus_rd_res->is_dual_isp = (uint32_t)c_ctx->flags.is_dual;
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	rc = cam_ife_hw_mgr_get_res(&c_ctx->free_res_list, &sfe_rd_res);
	if (rc) {
		CAM_ERR(CAM_ISP, "No more free hw mgr resource");
		goto err;
	}
	...
	sfe_acquire.tasklet = c_ctx->common.tasklet_info;
	sfe_acquire.sfe_rd.cdm_ops = c_ctx->cdm_ops;
	sfe_acquire.sfe_rd.is_offline = c_ctx->flags.is_offline;
	...
	list_for_each_entry(sfe_res_iterator, &c_ctx->res_list_ife_in_rd,
		list) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
...
acquire_successful:
	...
	cam_ife_hw_mgr_put_res(&c_ctx->res_list_ife_in_rd, &sfe_rd_res);
	...
	return 0;
put_res:
	cam_ife_hw_mgr_put_res(&c_ctx->free_res_list, &sfe_rd_res);
err:
	return rc;
}
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	if (c_ctx->flags.is_fe_enabled || c_ctx->flags.dsp_enabled)
	...
	c_ctx->flags.is_dual = (bool)in_port->usage_type;
	...
	list_for_each_entry(csid_res_iterator, &c_ctx->res_list_ife_csid,
		list) {
	...
	struct cam_isp_in_port_generic_info  *in_port)
{
	...
	if (!(in_port->ipp_count || in_port->lcr_count))
		need =  false;
	else if (c_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE &&
		((hw_mgr->csid_hw_caps[0].sfe_ipp_input_rdi_res && !in_port->usage_type) ||
		in_port->ife_rd_count))
		need =  false;

	CAM_DBG(CAM_ISP, "Need CSID PIX %u, Ctx_type: %u", need, c_ctx->ctx_type);
	return need;
}
	...
	bool                                 crop_enable)
{
	...
	for (i = csid_res->is_dual_isp; i >= 0 ; i--) {
		...
		if (i == CAM_ISP_HW_SPLIT_LEFT) {
			...
			c_ctx->right_hw_idx = hw_intf->hw_idx;
		}
	...
	if (csid_res->is_dual_isp && c_ctx->flags.need_csid_top_cfg) {
		for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
	...
	struct cam_isp_out_port_generic_info *out_port)
{
	...
	if (out_port) {
		if (cam_ife_hw_mgr_is_shdr_fs_rdi_res(out_port->res_type,
			c_ctx->flags.is_sfe_shdr, c_ctx->flags.is_sfe_fs)) {
	...
	if (!c_ctx->buf_done_controller && csid_acquire.buf_done_controller)
	...
	uint32_t                             *acquired_rdi_res)
{
	...
	CAM_DBG(CAM_ISP, "Ctx: %d rdi_res:%d ctx_type %d rc %d", c_ctx->ctx_index,
		res_id, c_ctx->ctx_type, rc);
end:
	return rc;
}
	...
	uint32_t                            *acquired_rdi_res)
{
	...
	CAM_DBG(CAM_ISP, "Ctx: %d  rdi: %d", c_ctx->ctx_index, res_id);
	return rc;
}
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	if (c_ctx->res_list_ife_in.res_type == CAM_ISP_RESOURCE_UNINT) {
		/* first acquire */
		c_ctx->res_list_ife_in.res_type = CAM_ISP_RESOURCE_SRC;
		c_ctx->res_list_ife_in.res_id = in_port->res_type;
		c_ctx->res_list_ife_in.is_dual_isp = in_port->usage_type;
	} else if ((c_ctx->res_list_ife_in.res_id !=
		in_port->res_type) && (!c_ctx->flags.is_fe_enabled))  {
	...
	uint32_t                           acquire_info_size)
{
	...
	CAM_DBG(CAM_ISP, "is_fe_enabled %d", c_ctx->flags.is_fe_enabled);
	...
	struct cam_ife_mgr_bw_data        *bw_data)
{
	...
	CAM_DBG(CAM_ISP,
		...
		c_ctx->flags.is_fe_enabled,
		c_ctx->flags.is_offline,
		c_ctx->flags.is_sfe_fs,
		c_ctx->flags.is_sfe_shdr,
		c_ctx->scratch_buf_info.num_fetches);
	...
	uint32_t                                   *acquired_hw_path)
{
	...
	isp_bus_rd_res = list_first_entry(&c_ctx->res_list_ife_in_rd,
		struct cam_isp_hw_mgr_res, list);
	if (!isp_bus_rd_res) {
	...
	struct cam_isp_in_port_generic_info        *in_port)
{
	...
	sfe_bus_rd_res = list_first_entry(&c_ctx->res_list_ife_in_rd,
		struct cam_isp_hw_mgr_res, list);
	...
	rc = cam_ife_hw_mgr_get_res(&c_ctx->free_res_list, &sfe_src_res);
	if (rc) {
		CAM_ERR(CAM_ISP, "No free resource");
		goto end;
	}
	...
	cam_ife_hw_mgr_put_res(&c_ctx->res_list_sfe_src, &sfe_src_res);
...
put_res:
	cam_ife_hw_mgr_put_res(&c_ctx->free_res_list, &sfe_src_res);
end:
	return rc;
}
	...
	struct cam_isp_in_port_generic_info        *in_port)
{
	...
	if (ife_ctx->is_offline) {
		csid_acquire.cb_priv = ife_hw_mgr;
		csid_acquire.event_cb =
			cam_ife_hw_mgr_event_handler_offline;
	} else {
		csid_acquire.cb_priv = ife_ctx;
		csid_acquire.event_cb = cam_ife_hw_mgr_event_handler;
	...
	uint32_t *acquired_rdi_res)
{
	...
	c_ctx->flags.dsp_enabled = (bool)in_port->dsp_mode;
	c_ctx->flags.is_dual = (bool)in_port->usage_type;

	/* Update aeb mode for the given in_port once */
	if ((in_port->aeb_mode) && (!c_ctx->flags.is_aeb_mode))
		c_ctx->flags.is_aeb_mode = true;
	...
	if (in_port->dynamic_sensor_switch_en)
		c_ctx->ctx_config |= CAM_IFE_CTX_CFG_DYNAMIC_SWITCH_ON;
	...
	enum cam_cdm_cb_status status, void *cookie)
{
	...
	hw_update_data = ctx->concr_ctx->cdm_userdata.hw_update_data;

	if (status == CAM_CDM_CB_STATUS_BL_SUCCESS) {
		req_id = *(uint64_t *)cookie;
		complete_all(&ctx->concr_ctx->config_done_complete);
		atomic_set(&ctx->concr_ctx->cdm_done, 1);
		ctx->concr_ctx->last_cdm_done_req = req_id;
		if (g_ife_hw_mgr.debug_cfg.per_req_reg_dump) {
			if (ctx->concr_ctx->cdm_userdata.request_id == req_id) {
				...
				CAM_INFO(CAM_ISP, "CDM delay, Skip dump req: %llu, cdm_req: %llu",
					req_id, ctx->concr_ctx->cdm_userdata.request_id);
			}
		}
		CAM_DBG(CAM_ISP,
			"CDM hdl=0x%x, udata=%pK, status=%d, cookie=%u ctx_index=%d cdm_req=%llu",
			 handle, userdata, status, req_id, ctx->concr_ctx->ctx_index,
			 ctx->concr_ctx->cdm_userdata.request_id);
	} else if (status == CAM_CDM_CB_STATUS_PAGEFAULT) {
		pf_args = (struct cam_hw_dump_pf_args *)cookie;
		if (ctx->concr_ctx->common.sec_pf_evt_cb)
			ctx->concr_ctx->common.sec_pf_evt_cb(ctx->concr_ctx->common.cb_priv, cookie);
	} else {
		CAM_WARN(CAM_ISP,
			"Called by CDM hdl=0x%x, udata=%pK, status=%d, cdm_req=%llu",
			 handle, userdata, status, ctx->concr_ctx->cdm_userdata.request_id);
	}
}
...
static inline void cam_ife_mgr_reset_streamon_scratch_cfg(
	struct cam_ife_hw_mgr_ctx *ctx)
{
	ctx->concr_ctx->scratch_buf_info.ife_scratch_config->skip_scratch_cfg_streamon = false;
	ctx->concr_ctx->scratch_buf_info.sfe_scratch_config->skip_scratch_cfg_streamon = false;
	ctx->concr_ctx->scratch_buf_info.ife_scratch_config->streamon_buf_mask = 0;
	...
	struct cam_hw_acquire_args *acquire_args)
{
	...
	ife_ctx->in_ports = kcalloc(ife_ctx->num_in_ports,
...
static int cam_ife_mgr_acquire_hw(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	if (!ife_mgr_ctx->in_ports) {
		CAM_ERR(CAM_ISP, "No memory available");
		rc = -ENOMEM;
		goto free_ctx;
	}

	/* Update in_ports structure */
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		rc = cam_ife_mgr_acquire_get_unified_structure(acquire_hw_info,
		...
		if (ife_ctx->flags.is_fe_enabled) {
			input_format_checker = ife_mgr_ctx->in_ports[i].format[0];
			for (j = 1; j < ife_mgr_ctx->in_ports[i].num_valid_vc_dt; j++) {
				if (ife_mgr_ctx->in_ports[i].format[j] != input_format_checker) {
					CAM_ERR(CAM_ISP,
						"Different input formats for FE use-cases not supported - formats vc0: %u vc%d: %u",
						input_format_checker, j, ife_mgr_ctx->in_ports[i].format[j]);
					rc = -EINVAL;
					goto free_mem;
				}
			}
		...
		cam_ife_hw_mgr_preprocess_port(ife_mgr_ctx, &ife_mgr_ctx->in_ports[i]);
		total_pix_port += ife_mgr_ctx->in_ports[i].ipp_count +
					ife_mgr_ctx->in_ports[i].ife_rd_count +
					ife_mgr_ctx->in_ports[i].lcr_count;
		total_rdi_port += ife_mgr_ctx->in_ports[i].rdi_count;
		total_pd_port += ife_mgr_ctx->in_ports[i].ppp_count;
		total_lite_port += ife_mgr_ctx->in_ports[i].lite_path_count;
	}
	...
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		CAM_DBG(CAM_ISP, "in_res_type %x", ife_mgr_ctx->in_ports[i].res_type);
		if (!ife_ctx->ctx_type) {
			...
			} else if (ife_mgr_ctx->in_ports[i].sfe_in_path_type) {
		...
		CAM_DBG(CAM_ISP,
			...
			ife_mgr_ctx->in_ports[i].res_type, ife_mgr_ctx->in_ports[i].sfe_in_path_type,
			ife_mgr_ctx->in_ports[i].sfe_ife_enable);
		...
		if (rc) {
			cam_ife_hw_mgr_print_acquire_info(ife_mgr_ctx,
				(ife_mgr_ctx->in_ports[i].ipp_count +
				ife_mgr_ctx->in_ports[i].ife_rd_count +
				ife_mgr_ctx->in_ports[i].lcr_count),
				ife_mgr_ctx->in_ports[i].ppp_count,
				ife_mgr_ctx->in_ports[i].rdi_count, rc);
			goto free_res;
		}
	}
	...
	if (ife_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
		rc = cam_ife_mgr_update_core_info_to_cpas(ife_mgr_ctx, true);
		if (rc)
			goto free_res;
	}
...
free_mem:
	if (ife_mgr_ctx->in_ports) {
		for (i = 0; i < acquire_hw_info->num_inputs; i++) {
			...
			ife_mgr_ctx->in_ports[i].data = NULL;
		}

		kfree(ife_mgr_ctx->in_ports);
		ife_mgr_ctx->in_ports = NULL;
	}
free_ctx:
	...
	for (i = 0; i < acquire_args->num_acq; i++) {
		...
		if (!IS_ERR(in_port)) {
			...
			cam_ife_mgr_acquire_get_unified_dev_str(in_port,
				&gen_port_info[i]);
			cam_ife_hw_mgr_preprocess_port(ife_mgr_ctx,
				&gen_port_info[i]);
	...
	struct cam_isp_prepare_hw_update_data *hw_update_data)
{
	...
	CAM_INFO(CAM_ISP, "ctx: %d req_id:%llu config_valid[BW VFE_CLK SFE_CLK]:[%d %d %d]",
		ctx->concr_ctx->ctx_index, request_id, hw_update_data->bw_clk_config.bw_config_valid,
					...
					void *config_hw_args)
{
	...
	if (!c_ctx->flags.ctx_in_use || !c_ctx->cdm_cmd) {
		CAM_ERR(CAM_ISP,
			"Invalid context parameters : ctx_in_use=%d, cdm_cmd=%pK",
			c_ctx->flags.ctx_in_use, c_ctx->cdm_cmd);
		return -EPERM;
	}
	 ...
	 */
	if (c_ctx->try_recovery_cnt &&
		(cfg->request_id > (c_ctx->recovery_req_id + 1))) {
		c_ctx->try_recovery_cnt = 0;
		c_ctx->recovery_req_id = 0;
		CAM_DBG(CAM_ISP,
	...
	hw_update_data->isp_mgr_ctx = hw_mgr_ctx;
	...
	if (cfg->reapply_type && cfg->cdm_reset_before_apply) {
		if (c_ctx->last_cdm_done_req < cfg->request_id) {
			is_cdm_hung = !cam_cdm_detect_hang_error(c_ctx->cdm_handle);
			CAM_ERR_RATE_LIMIT(CAM_ISP,
				"CDM callback not received for req: %lld, last_cdm_done_req: %lld, is_cdm_hung: %d",
				cfg->request_id, c_ctx->last_cdm_done_req,
				is_cdm_hung);

			if (!is_cdm_hung)
				cam_cdm_dump_debug_registers(c_ctx->cdm_handle);

			rc = cam_cdm_reset_hw(c_ctx->cdm_handle);
			if (rc) {
				CAM_ERR_RATE_LIMIT(CAM_ISP,
					"CDM reset unsuccessful for req: %lld. ctx: %d, rc: %d",
					cfg->request_id, c_ctx->ctx_index, rc);
	...
	if (hw_update_data->drv_config_valid) {
		rc = cam_isp_blob_drv_config(hw_mgr_ctx, cfg->request_id, hw_update_data);
		if (rc) {
	...
	if (hw_update_data->bw_clk_config.ife_clock_config_valid) {
		rc = cam_isp_blob_ife_clock_update((struct cam_isp_clock_config *)
			&hw_update_data->bw_clk_config.ife_clock_config, hw_mgr_ctx);
		if (rc) {
	...
	if (hw_update_data->bw_clk_config.sfe_clock_config_valid) {
		rc = cam_isp_blob_sfe_clock_update((struct cam_isp_clock_config *)
			&hw_update_data->bw_clk_config.sfe_clock_config, hw_mgr_ctx);
		if (rc) {
			CAM_ERR(CAM_PERF, "Clock Update Failed, rc=%d", rc);
			return rc;
		}
	...
	if (hw_update_data->bw_clk_config.bw_config_valid) {
		if (c_ctx->bw_config_version == CAM_ISP_BW_CONFIG_V1) {
			rc = cam_isp_blob_bw_update(
				(struct cam_isp_bw_config *)
				&hw_update_data->bw_clk_config.bw_config, hw_mgr_ctx);
			if (rc) {
				CAM_ERR(CAM_PERF, "Bandwidth Update Failed rc: %d", rc);
				return rc;
			}
		} else if ((c_ctx->bw_config_version == CAM_ISP_BW_CONFIG_V2) ||
			(c_ctx->bw_config_version == CAM_ISP_BW_CONFIG_V3)) {
			...
			CAM_ERR(CAM_PERF, "Invalid bw config version: %d", c_ctx->bw_config_version);
		}
	}

	/* Apply the updated values in top layer to the HW*/
	rc = cam_ife_mgr_finish_clk_bw_update(hw_mgr_ctx, cfg->request_id, false);
	if (rc) {
		CAM_ERR(CAM_ISP, "Failed in finishing clk/bw update rc: %d", rc);
		cam_ife_mgr_print_blob_info(hw_mgr_ctx, cfg->request_id, hw_update_data);
		return rc;
	}
	...
	if (cfg->num_hw_update_entries > 0) {
		...
		if (cfg->init_packet || hw_update_data->mup_en ||
			(c_ctx->ctx_config & CAM_IFE_CTX_CFG_SW_SYNC_ON)) {
			...
			if (rem_jiffies == 0) {
				...
				rc = cam_cdm_detect_hang_error(c_ctx->cdm_handle);
				if (rc < 0) {
				...
				if (hw_update_data->mup_en) {
					c_ctx->current_mup = hw_update_data->mup_val;
		...
		cam_ife_mgr_send_frame_event(cfg->request_id, c_ctx->ctx_index);
	} else {
...
static int cam_ife_mgr_stop_hw(void *hw_mgr_priv, void *stop_hw_args)
{
	...
	CAM_DBG(CAM_ISP, " ctx id:%d hw mgr ctx %d", ctx->ctx_index,hw_mgr_ctx->ctx_idx);
	...
	if (csid_halt_type == CAM_CSID_HALT_IMMEDIATELY)
		cam_ife_mgr_csid_change_halt_mode(hw_mgr_ctx,
	...
	if (stop_isp->is_internal_stop)
		cam_ife_mgr_finish_clk_bw_update(hw_mgr_ctx, 0, true);
	...
	CAM_DBG(CAM_ISP,
		"Stop success for ctx id:%d hw mgr ctx %d rc :%d", ctx->ctx_index, hw_mgr_ctx->ctx_idx, rc);
...
static int cam_ife_mgr_restart_hw(void *start_hw_args)
{
	...
	rc = cam_ife_mgr_csid_start_hw(hw_mgr_ctx, CAM_IFE_PIX_PATH_RES_MAX, false);
	if (rc) {
	...
	uint32_t *core_idx)
{
	...
	for (i = 0; i < c_ctx->num_base; i++) {
		if (c_ctx->base[i].hw_type != hw_type)
			continue;

		if (c_ctx->base[i].split_id == split_id) {
			CAM_DBG(CAM_ISP, "Found core: %u for split_id: %d hw_type: %d",
				c_ctx->base[i].idx, split_id, hw_type);
			*core_idx = c_ctx->base[i].idx;
			goto end;
		}
	}
	...
	struct cam_ife_hw_mgr_ctx *ctx)
{
	...
	uint32_t hw_idx = 0;
	struct cam_ife_hw_concrete_ctx   *c_ctx = ctx->concr_ctx;
	struct cam_ife_hw_mgr           *hw_mgr = c_ctx->hw_mgr;
	...
	for (i = 0; i < c_ctx->num_base; i++) {
		hw_idx = c_ctx->base[i].idx;

		switch (c_ctx->base[i].hw_type) {
...
static int cam_ife_mgr_start_hw(void *hw_mgr_priv, void *start_hw_args)
{
	...
	hw_mgr_ctx = (struct cam_ife_hw_mgr_ctx *)start_isp->hw_config.ctxt_to_hw_map;
	if (!hw_mgr_ctx) {
		CAM_ERR(CAM_ISP, "Invalid context is used");
		return -EPERM;
	}
	...
	cam_ife_hw_mgr_set_hw_debug_config(hw_mgr_ctx);
		...
		list_for_each_entry(hw_mgr_res, &ctx->res_list_ife_csid,
			...
			for (i = 0; i < CAM_ISP_HW_SPLIT_MAX; i++) {
				...
				if (ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
					...
					rc = cam_ife_mgr_find_core_idx(i, hw_mgr_ctx, CAM_ISP_HW_TYPE_SFE,
...
start_only:
	...
	for (i = 0; i < max_ife_out_res; i++) {
		...
		case CAM_ISP_IFE_OUT_RES_RDI_2:
		case CAM_ISP_IFE_OUT_RES_RDI_3:
			if (!res_rdi_context_set && cam_isp_is_ctx_primary_rdi(hw_mgr_ctx)) {
				hw_mgr_res->hw_res[0]->is_rdi_primary_res =
					cam_isp_is_ctx_primary_rdi(hw_mgr_ctx);
	...
	if (ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
		...
		for (i = 0; i < max_sfe_out_res; i++) {
			...
			rc = cam_ife_hw_mgr_start_hw_res(
				&ctx->res_list_sfe_out[i], hw_mgr_ctx);
			if (rc) {
	...
	if ((ctx->flags.is_sfe_fs) || (ctx->flags.is_sfe_shdr)) {
		rc = cam_ife_mgr_prog_default_settings(true, hw_mgr_ctx);
		if (rc)
			goto err;

		/* Reset streamon related scratch buffer config */
		cam_ife_mgr_reset_streamon_scratch_cfg(hw_mgr_ctx);
	}

	CAM_DBG(CAM_ISP, "START CSID HW ... in ctx id:%d",
		ctx->ctx_index);
	/* Start the IFE CSID HW devices */
	rc = cam_ife_mgr_csid_start_hw(hw_mgr_ctx, primary_rdi_csid_res,
...
static int cam_ife_mgr_reset(void *hw_mgr_priv, void *hw_reset_args)
{
	...
	hw_mgr_ctx = (struct cam_ife_hw_mgr_ctx *)reset_args->ctxt_to_hw_map;
	if (!hw_mgr_ctx) {
		CAM_ERR(CAM_ISP, "Invalid context is used");
		return -EPERM;
	}
	...
	if (hw_mgr->csid_global_reset_en) {
		CAM_DBG(CAM_ISP, "Path reset not supported, ctx_idx: %u", c_ctx->ctx_index);
		return 0;
	}

	CAM_DBG(CAM_ISP, "Reset CSID and VFE, ctx_idx: %u", c_ctx->ctx_index);

	rc = cam_ife_hw_mgr_reset_csid(hw_mgr_ctx, CAM_IFE_CSID_RESET_PATH);
	...
	for (i = 0; i < c_ctx->num_base; i++) {
		rc = cam_ife_mgr_reset_vfe_hw(hw_mgr, c_ctx->base[i].idx);
		if (rc) {
			CAM_ERR(CAM_ISP, "Failed to reset VFE:%d rc: %d ctx_idx: %u",
				c_ctx->base[i].idx, rc, c_ctx->ctx_index);
			goto end;
		}
	}
					...
					void *release_hw_args)
{
	...
	if (c_ctx->ctx_type == CAM_IFE_CTX_TYPE_SFE) {
		rc = cam_ife_mgr_update_core_info_to_cpas(hw_mgr_ctx, false);
		if (rc)
	...
	/* reset base info */
	c_ctx->num_base = 0;
	memset(c_ctx->base, 0, sizeof(c_ctx->base));

	/* release cdm handle */
	cam_cdm_release(c_ctx->cdm_handle);

	/* clean context */
	list_del_init(&c_ctx->list);
	c_ctx->cdm_handle = 0;
	c_ctx->cdm_hw_idx = -1;
	c_ctx->cdm_ops = NULL;
	c_ctx->num_reg_dump_buf = 0;
	c_ctx->ctx_type = CAM_IFE_CTX_TYPE_NONE;
	c_ctx->ctx_config = 0;
	c_ctx->num_reg_dump_buf = 0;
	c_ctx->last_cdm_done_req = 0;
	c_ctx->left_hw_idx = CAM_IFE_CSID_HW_NUM_MAX;
	c_ctx->right_hw_idx = CAM_IFE_CSID_HW_NUM_MAX;
	c_ctx->scratch_buf_info.num_fetches = 0;
	c_ctx->num_acq_vfe_out = 0;
	c_ctx->num_acq_sfe_out = 0;
	c_ctx->buf_done_controller = NULL;
	kfree(c_ctx->scratch_buf_info.sfe_scratch_config);
	kfree(c_ctx->scratch_buf_info.ife_scratch_config);
	c_ctx->scratch_buf_info.sfe_scratch_config = NULL;
	c_ctx->scratch_buf_info.ife_scratch_config = NULL;
	c_ctx->try_recovery_cnt = 0;
	c_ctx->recovery_req_id = 0;
	...
	cam_ife_mgr_free_cdm_cmd(&c_ctx->cdm_cmd);
	...
	memset(&c_ctx->ts, 0, sizeof(struct timespec64));
...
static uint32_t cam_ife_hw_mgr_get_sfe_sys_cache_id(uint32_t exp_type,
	struct cam_ife_hw_mgr_ctx  *hw_mgr_ctx, uint32_t hw_idx)
{
	...
	for (i = 0; i < vfe_out_config->num_ports; i++) {
		...
		if (is_sfe_rd) {
			rc = cam_isp_blob_sfe_rd_update(blob_type,
				...
				&total_used_bytes, hw_mgr_ctx, blob_info, wm_config);
			if (rc)
				return rc;
		...
		if (hw_type == CAM_ISP_HW_TYPE_SFE) {
			...
			if ((ctx->flags.is_sfe_shdr) &&
				...
				rc = cam_ife_hw_mgr_update_scratch_offset(hw_mgr_ctx, wm_config);
				if (rc)
					return rc;
			}
	...
	uint32_t                           blob_type)
{
	...
	if (prepare_hw_data->packet_opcode_type == CAM_ISP_PACKET_INIT_DEV) {
		rc = cam_isp_config_rdi_lcr_csid_init_params(hw_mgr_ctx,
	...
	rc = cam_isp_update_ife_pdaf_cfg(hw_mgr_ctx, prepare, blob_info,
...
static int cam_isp_validate_scratch_buffer_blob(
	...
	struct cam_isp_sfe_init_scratch_buf_config *scratch_config)
{
	struct cam_ife_hw_concrete_ctx *ctx = ife_mgr_ctx->concr_ctx;
	...
	uint32_t blob_type, uint32_t blob_size, uint8_t *blob_data)
{
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_BW_CONFIG: {
		...
		ife_mgr_ctx->concr_ctx->bw_config_version = CAM_ISP_BW_CONFIG_V1;
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_BW_CONFIG_V2: {
		...
		ife_mgr_ctx->concr_ctx->bw_config_version = CAM_ISP_BW_CONFIG_V2;
		prepare_hw_data->bw_clk_config.bw_config_valid = true;
	}
		break;
	case CAM_ISP_GENERIC_BLOB_TYPE_BW_CONFIG_V3: {
		...
		ife_mgr_ctx->concr_ctx->bw_config_version = CAM_ISP_BW_CONFIG_V3;
		prepare_hw_data->bw_clk_config.bw_config_valid = true;
	}
		break;
	...
	uint32_t blob_type, uint32_t blob_size, uint8_t *blob_data)
{
	...
	case CAM_ISP_GENERIC_BLOB_TYPE_DYNAMIC_MODE_SWITCH: {
		...
		prepare_hw_data = (struct cam_isp_prepare_hw_update_data  *)
			prepare->priv;
		mup_config = (struct cam_isp_mode_switch_info *)blob_data;
		if (ife_mgr_ctx->concr_ctx->flags.is_sfe_shdr) {
			ife_mgr_ctx->concr_ctx->scratch_buf_info.sfe_scratch_config->updated_num_exp =
				mup_config->num_expoures;
...
static inline bool cam_isp_sfe_validate_for_scratch_buf_config(
	uint32_t res_idx, struct cam_ife_hw_mgr_ctx  *hw_mgr_ctx,
	bool default_settings)
{
	uint32_t curr_num_exp;

	/* check for num exposures for static mode but using RDI1-2 without RD1-2 */
	if (res_idx >= hw_mgr_ctx->concr_ctx->scratch_buf_info.num_fetches)
		return true;

	if (default_settings)
		curr_num_exp = hw_mgr_ctx->concr_ctx->curr_num_exp;
	else
		curr_num_exp = hw_mgr_ctx->concr_ctx->scratch_buf_info.sfe_scratch_config->updated_num_exp;

	/* check for num exposures for dynamic mode */
	if ((hw_mgr_ctx->concr_ctx->ctx_config &
		CAM_IFE_CTX_CFG_DYNAMIC_SWITCH_ON) &&
	...
	struct cam_ife_hw_mgr_ctx            *hw_mgr_ctx)
{
	...
	CAM_DBG(CAM_ISP, "num_ports: %u",
		c_ctx->scratch_buf_info.sfe_scratch_config->num_config);
	...
	for (i = 0; i < CAM_SFE_FE_RDI_NUM_MAX; i++) {
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			if (cam_isp_sfe_validate_for_scratch_buf_config(
				(res_id - CAM_ISP_SFE_OUT_RES_RDI_0), hw_mgr_ctx, false))
				continue;
			...
			buf_info = &hw_mgr_ctx->concr_ctx->scratch_buf_info.sfe_scratch_config->buf_info[
	...
	list_for_each_entry(hw_mgr_res, res_list_in_rd, list) {
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			if (cam_isp_sfe_validate_for_scratch_buf_config(
				(res_id - CAM_ISP_SFE_IN_RD_0), hw_mgr_ctx, false))
				continue;
			...
			cpu_addr = kmd_buf_info->cpu_addr +
				kmd_buf_info->used_bytes  / 4 +
				io_cfg_used_bytes / 4;
			buf_info = &c_ctx->scratch_buf_info.sfe_scratch_config->buf_info[
				res_id - CAM_ISP_SFE_IN_RD_0];
	...
	struct cam_ife_hw_mgr_ctx            *hw_mgr_ctx)
{
	...
	for (i = 0; i < hw_mgr_ctx->concr_ctx->scratch_buf_info.ife_scratch_config->num_config; i++) {
		/*
		...
		res_id = hw_mgr_ctx->concr_ctx->scratch_buf_info.ife_scratch_config->buf_info[i].res_id & 0xFF;
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			cpu_addr = kmd_buf_info->cpu_addr +
				kmd_buf_info->used_bytes / 4 + io_cfg_used_bytes / 4;
			buf_info = &hw_mgr_ctx->concr_ctx->scratch_buf_info.ife_scratch_config->buf_info[i];
...
static int cam_ife_mgr_csid_add_reg_update(struct cam_ife_hw_mgr_ctx *hw_mgr_ctx,
	struct cam_hw_prepare_update_args *prepare,
	struct cam_kmd_buf_info *kmd_buf)
{
	int                                   i;
...
static int cam_ife_mgr_isp_add_reg_update(struct cam_ife_hw_mgr_ctx *hw_mgr_ctx,
	struct cam_hw_prepare_update_args *prepare,
	struct cam_kmd_buf_info *kmd_buf)
{
	int i;
	...
	if ((sfe_res_info->sfe_rdi_cfg_mask) !=
		...
		if (opcode_type == CAM_ISP_PACKET_UPDATE_DEV) {
			...
			rc = cam_isp_sfe_add_scratch_buffer_cfg(
				...
				&ctx->res_list_ife_in_rd, hw_mgr_ctx);
			if (rc)
				goto end;
	...
	if ((ife_res_info->ife_scratch_cfg_mask) !=
		...
		if (opcode_type == CAM_ISP_PACKET_UPDATE_DEV) {
			...
			rc = cam_isp_ife_add_scratch_buffer_cfg(
				...
				kmd_buf, ctx->res_list_ife_out, hw_mgr_ctx);
			if (rc)
				goto end;
		} else if (opcode_type == CAM_ISP_PACKET_INIT_DEV) {
	...
	void *prepare_hw_update_args)
{
	...
	hw_mgr_ctx = (struct cam_ife_hw_mgr_ctx *)prepare->ctxt_to_hw_map;
	if (!hw_mgr_ctx) {
		CAM_ERR(CAM_ISP, "Invalid hw mgr ctx");
		return -EINVAL;
	}
	...
	cam_ife_hw_mgr_check_if_scratch_is_needed(hw_mgr_ctx, &check_for_scratch);
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		rc = cam_ife_hw_mgr_update_cmd_buffer(hw_mgr_ctx, prepare,
		...
		if ((check_for_scratch.validate_for_sfe) &&
			...
			rc = cam_ife_hw_mgr_sfe_scratch_buf_update(
				prepare_hw_data->packet_opcode_type,
				i, &prepare_hw_data->kmd_cmd_buff_info, prepare, hw_mgr_ctx, sfe_res_info);
			if (rc)
				goto end;
		}
		...
		if ((check_for_scratch.validate_for_ife) &&
			...
			rc = cam_ife_hw_mgr_ife_scratch_buf_update(
				prepare_hw_data->packet_opcode_type,
				i, &prepare_hw_data->kmd_cmd_buff_info, prepare, hw_mgr_ctx, ife_res_info);
			if (rc)
				goto end;
		}
	...
	if (hw_mgr->csid_rup_en)
		rc = cam_ife_mgr_csid_add_reg_update(hw_mgr_ctx,
			prepare, &prepare_hw_data->kmd_cmd_buff_info);

	else
		rc = cam_ife_mgr_isp_add_reg_update(hw_mgr_ctx,
			prepare, &prepare_hw_data->kmd_cmd_buff_info);
...
static inline void cam_ife_hw_mgr_stop_bus_rd_for_res(struct cam_ife_hw_mgr_ctx *hw_mgr_ctx,
	uint32_t res_id)
{
	struct cam_isp_hw_mgr_res *isp_hw_res;
	struct cam_ife_hw_concrete_ctx  *ctx=hw_mgr_ctx->concr_ctx;
...
static void cam_ife_hw_mgr_stop_pf_hw_res(struct cam_ife_hw_mgr_ctx *hw_mgr_ctx,
	uint32_t res_id, uint32_t hw_type)
{
	...
	if (hw_type == CAM_ISP_HW_TYPE_VFE) {
		if (cam_ife_hw_mgr_is_ife_out_port(res_id)) {
			...
			cam_ife_hw_mgr_stop_bus_rd_for_res(hw_mgr_ctx, res_id);
...
static void cam_ife_mgr_pf_dump(struct cam_ife_hw_mgr_ctx *hw_mgr_ctx)
{
	struct cam_isp_hw_mgr_res      *hw_mgr_res;
	struct cam_hw_intf             *hw_intf;	
	struct cam_ife_hw_concrete_ctx  *ctx=hw_mgr_ctx->concr_ctx;
	...
	/* dump the registers  */
	rc = cam_ife_mgr_handle_reg_dump(hw_mgr_ctx, ctx->reg_dump_buf_desc,
	...
	struct cam_hw_cmd_args *hw_cmd_args)
{
	...
	hw_mgr_ctx = (struct cam_ife_hw_mgr_ctx *)hw_cmd_args->ctxt_to_hw_map;
	if(!hw_mgr_ctx) {
	...
	for (i = 0; i < ctx->num_base; i++) {
		...
		if (!g_ife_hw_mgr.hw_pid_support) {
			if (ctx->base[i].split_id == CAM_ISP_HW_SPLIT_LEFT)
				cam_ife_mgr_pf_dump_mid_info(hw_mgr_ctx, hw_cmd_args, hw_intf_data,
					packet);
			continue;
		}
			...
			if (hw_intf_data->hw_pid[j] == pf_args->pf_smmu_info->pid) {
				*ctx_found = true;
				...
				if (ctx->flags.pf_mid_found) {
					cam_ife_hw_mgr_stop_pf_hw_res(hw_mgr_ctx, ctx->pf_info.out_port_id,
						ctx->base[i].hw_type);
	...
	cam_ife_mgr_pf_dump(hw_mgr_ctx);
...
static int cam_ife_mgr_configure_scratch_for_sfe(
	bool is_streamon, struct cam_ife_hw_mgr_ctx *hw_mgr_ctx)
{
	int i, j, res_id, rc = 0;
	struct cam_isp_hw_mgr_res           *hw_mgr_res;
	struct cam_ife_sfe_scratch_buf_info *buf_info;
	...
	for (i = 0; i < CAM_SFE_FE_RDI_NUM_MAX; i++) {
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			if (cam_isp_sfe_validate_for_scratch_buf_config(
				(res_id - CAM_ISP_SFE_OUT_RES_RDI_0), hw_mgr_ctx, true))
				continue;
	...
	list_for_each_entry(hw_mgr_res, res_list_in_rd, list) {
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			if (cam_isp_sfe_validate_for_scratch_buf_config(
				(res_id - CAM_ISP_SFE_IN_RD_0), hw_mgr_ctx, true))
				continue;
	...
	bool is_streamon, struct cam_ife_hw_mgr_ctx *hw_mgr_ctx)
{
	...
	rc = cam_ife_mgr_configure_scratch_for_sfe(is_streamon, hw_mgr_ctx);
	if (rc)
		goto end;

	/* Check for IFE scratch buffers */
	if (ctx->scratch_buf_info.ife_scratch_config->num_config) {
		rc = cam_ife_mgr_configure_scratch_for_ife(is_streamon, hw_mgr_ctx);
		if (rc)
			goto end;
	}
	...
	if (!is_streamon) {
		rc = cam_isp_config_csid_rup_aup(hw_mgr_ctx);
		if (rc)
			CAM_ERR(CAM_ISP,
	...
	void *dump_struct, uint8_t *addr_ptr)
{
	...
	if (!list_empty(&ctx->res_list_sfe_src)) {
		hw_mgr_res = list_first_entry(&ctx->res_list_sfe_src,
			struct cam_isp_hw_mgr_res, list);
	...
	*addr++ = ctx->flags.is_sfe_shdr;
	*addr++ = ctx->flags.is_sfe_fs;
	*addr++ = ctx->flags.dsp_enabled;
	*addr++ = ctx->flags.is_offline;
...
static int cam_ife_mgr_cmd(void *hw_mgr_priv, void *cmd_args)
{
	...
	if (!hw_mgr_ctx || !ctx || !ctx->flags.ctx_in_use) {
	...
	case CAM_HW_MGR_CMD_INTERNAL:
		...
		case CAM_ISP_HW_MGR_CMD_PROG_DEFAULT_CFG:
			rc = cam_ife_mgr_prog_default_settings(false, hw_mgr_ctx);
			break;
		case CAM_ISP_HW_MGR_GET_SOF_TS:
			rc = cam_ife_mgr_cmd_get_sof_timestamp(hw_mgr_ctx,
		...
		case CAM_ISP_HW_MGR_DUMP_STREAM_INFO:
			rc = cam_common_user_dump_helper(
				...
				cam_ife_mgr_user_dump_stream_info, hw_mgr_ctx,
...
static int cam_ife_mgr_recover_hw(void *priv, void *data)
{
	...
	for (i = 0; i < recovery_data->no_of_context; i++) {
		c_ctx = recovery_data->affected_ctx[i];
		if (recovery_data->id[i] != atomic_read(&c_ctx->recovery_id)) {
			CAM_INFO(CAM_ISP, "recovery for ctx:%u error-type:%d cancelled",
				c_ctx->ctx_index, error_type);
			cancel = true;
		}
	}
	if (cancel)
	...
	case CAM_ISP_HW_ERROR_VIOLATION:
		...
		for (i = 0; i < recovery_data->no_of_context; i++) {
			c_ctx = recovery_data->affected_ctx[i];
			idx = c_ctx->served_ctx_id[c_ctx->served_ctx_w];
			if (!ife_hwr_mgr->virt_ctx_pool[idx].ctx_in_use) {
				CAM_ERR(CAM_ISP, "Context[%d] is Not in use", idx);
				goto end;
			}
			ife_ctx = &ife_hwr_mgr->virt_ctx_pool[idx];
			stop_args.ctxt_to_hw_map = ife_ctx;
		...
		for (i = 0; i < recovery_data->no_of_context; i++) {
			...
			((struct cam_ife_hw_mgr_ctx *)
				start_args.ctxt_to_hw_map)->concr_ctx = c_ctx;
			rc = cam_ife_hw_mgr_reset_csid(
				(struct cam_ife_hw_mgr_ctx *)
					 start_args.ctxt_to_hw_map,
				CAM_IFE_CSID_RESET_PATH);

			if (rc) {
				CAM_ERR(CAM_ISP, "Failed RESET, ctx_idx: %u", c_ctx->ctx_index);
				return rc;
			}
		}
		...
		for (i = 0; i < recovery_data->no_of_context; i++) {
			...
			ife_ctx = &ife_hwr_mgr->virt_ctx_pool[idx];
			start_args.ctxt_to_hw_map = ife_ctx;
...
static bool cam_ife_hw_mgr_is_ctx_affected(
	struct cam_ife_hw_concrete_ctx   *ife_hw_concrete_ctx,
	...
	uint32_t                     size)
{
	...
	bool                  rc = false;
	uint32_t              i = 0, j = 0;
	uint32_t              max_idx =  ife_hw_concrete_ctx->num_base;
	...
	for (i = 0; i < max_idx; i++) {
		if (affected_core[ife_hw_concrete_ctx->base[i].idx])
			rc = true;
		else {
			ctx_affected_core_idx[j] = ife_hw_concrete_ctx->base[i].idx;
			j = j + 1;
		}
	}
	...
	struct cam_ife_hw_mgr_ctx            *ife_mgr_ctx)
{
	...
	cam_hw_event_cb_func ife_hw_irq_cb =
		ife_mgr_ctx->event_cb[CAM_ISP_HW_SECONDARY_EVENT];
	...
	if (err_type & CAM_ISP_HW_ERROR_CSID_SENSOR_FRAME_DROP) {
		...
		rc = ife_hw_irq_cb(ife_mgr_ctx->cb_priv,
	...
	struct cam_isp_hw_event_info   *event_info)
{
	...
	if (event_info->is_secondary_evt) {
		rc = cam_ife_hw_mgr_handle_csid_secondary_err_evts(err_type, event_info, ife_mgr_ctx);
		if (rc)
			CAM_ERR(CAM_ISP,
	...
	struct cam_isp_hw_event_info     *event_info)
{
	...
	ife_hwr_irq_rup_cb =
		ife_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_REG_UPDATE];
	...
	case CAM_IFE_PIX_PATH_RES_PPP:
		if (atomic_read(&ctx->overflow_pending))
			break;
		ife_hwr_irq_rup_cb(ife_mgr_ctx->cb_priv,
	...
	struct cam_isp_hw_event_info     *event_info)
{
	...
	ife_hwr_irq_rup_cb = ife_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_EOF];
	...
	struct cam_isp_hw_event_info         *event_info)
{
	...
	cam_hw_event_cb_func ife_hw_irq_sof_cb    =
		ife_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_SOF];
	...
	if (event_info->is_secondary_evt) {
		...
		rc = ife_hw_irq_sof_cb(ife_mgr_ctx->cb_priv,
			CAM_ISP_HW_SECONDARY_EVENT, (void *)&sec_evt_data);
		goto end;
	}
	...
	case CAM_IFE_PIX_PATH_RES_PPP:
		...
		if (ctx->ctx_config &
			...
			if (ctx->flags.is_offline)
				...
				ife_mgr_ctx, &sof_done_event_data.timestamp,
				&sof_done_event_data.boot_time, NULL);
		}

		ife_hw_irq_sof_cb(ife_mgr_ctx->cb_priv,
	...
	struct cam_isp_hw_event_info         *event_info)
{
	...
	cam_hw_event_cb_func ife_hw_irq_epoch_cb =
		ife_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_EPOCH];
	...
	if (event_info->is_secondary_evt) {
		...
		rc = ife_hw_irq_epoch_cb(ife_mgr_ctx->cb_priv,
			CAM_ISP_HW_SECONDARY_EVENT, (void *)&sec_evt_data);
		goto end;
	}
	...
	case CAM_IFE_PIX_PATH_RES_PPP:
		...
		ife_hw_irq_epoch_cb(ife_mgr_ctx->cb_priv,
...
static int cam_ife_hw_mgr_handle_sfe_hw_error(
	struct cam_ife_hw_concrete_ctx       *sfe_hw_ctx,
	struct cam_isp_hw_event_info    *event_info)
{
	struct cam_isp_hw_error_event_info     *err_evt_info;
	struct cam_isp_hw_error_event_data      error_event_data = {0};
	...
	if (!event_info->event_data) {
		CAM_ERR(CAM_ISP,
			...
			event_info->hw_idx, sfe_hw_ctx->ctx_index);
		return -EINVAL;
	}
	...
	struct cam_isp_hw_event_info      *event_info)
{
	...
	if (!event_info->event_data) {
		CAM_ERR(CAM_ISP,
			...
			event_info->hw_idx, ife_ctx->concr_ctx->ctx_index);
		return -EINVAL;
	}
	...
	if (event_info->res_type ==
		...
		!ife_ctx->concr_ctx->flags.is_rdi_only_context &&
		event_info->res_id !=
		CAM_ISP_HW_VFE_IN_CAMIF)
		cam_ife_hw_mgr_handle_hw_dump_info(ife_ctx, event_info);
	...
	struct cam_isp_hw_event_info            *event_info)
{
	...
	struct cam_isp_hw_reg_update_event_data  rup_event_data;
	struct cam_ife_hw_concrete_ctx     *c_ctx =
		((struct cam_ife_hw_mgr_ctx *)ife_hw_mgr_ctx)->concr_ctx;

	ife_hwr_irq_rup_cb =
		ife_hw_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_REG_UPDATE];
	...
	case CAM_ISP_HW_VFE_IN_CAMIF:
		...
		if (atomic_read(&c_ctx->overflow_pending))
			break;
		ife_hwr_irq_rup_cb(ife_hw_mgr_ctx->cb_priv,
	...
	struct cam_isp_hw_event_info         *event_info)
{
	...
	struct cam_ife_hw_concrete_ctx       *ife_ctx = ife_hw_mgr_ctx->concr_ctx;

	ife_hw_irq_epoch_cb =
		ife_hw_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_EPOCH];
	...
	case CAM_ISP_HW_VFE_IN_CAMIF:
		...
		epoch_done_event_data.frame_id_meta = event_info->reg_val;
		ife_hw_irq_epoch_cb(ife_hw_mgr_ctx->cb_priv,
			CAM_ISP_HW_EVENT_EPOCH, (void *)&epoch_done_event_data);
	...
	struct cam_isp_hw_event_info         *event_info)
{
	...
	ife_hw_irq_sof_cb =
		ife_hw_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_SOF];

	switch (event_info->res_id) {
	case CAM_ISP_HW_VFE_IN_CAMIF:
	case CAM_ISP_HW_VFE_IN_RD:
		/* if frame header is enabled reset qtimer ts */
		if (ife_hw_mgr_ctx->concr_ctx->ctx_config &
			CAM_IFE_CTX_CFG_FRAME_HEADER_TS) {
			sof_done_event_data.timestamp = 0x0;
			ktime_get_boottime_ts64(&ts);
			sof_done_event_data.boot_time =
		...
		ife_hw_irq_sof_cb(ife_hw_mgr_ctx->cb_priv,
			CAM_ISP_HW_EVENT_SOF, (void *)&sof_done_event_data);
	...
	struct cam_isp_hw_event_info         *event_info)
{
	...
	ife_hw_irq_eof_cb =
		ife_hw_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_EOF];
	...
	case CAM_ISP_HW_VFE_IN_CAMIF:
		...
		ife_hw_irq_eof_cb(ife_hw_mgr_ctx->cb_priv,
	...
	uint32_t res_id, uint32_t last_consumed_addr)
{
	...
	switch (hw_type) {
	case CAM_ISP_HW_TYPE_VFE:
		if (ife_ctx->scratch_buf_info.ife_scratch_config->num_config)
	...
	struct cam_isp_hw_event_info     *event_info)
{
	...
	ife_hwr_irq_wm_done_cb =
		ife_hw_mgr_ctx->event_cb[CAM_ISP_HW_EVENT_DONE];
	...
	for (i = 0; i < compdone_evt_info->num_res; i++) {
		CAM_DBG(CAM_ISP,
			...
			compdone_evt_info->last_consumed_addr[i], ife_ctx->ctx_index);

		/* Check scratch for sHDR/FS use-cases */
		if (ife_ctx->flags.is_sfe_fs || ife_ctx->flags.is_sfe_shdr) {
	...
	if (buf_done_event_data.num_handles > 0 && ife_hwr_irq_wm_done_cb) {
		CAM_DBG(CAM_ISP,
			"Notify ISP context for %u handles in conr ctx: %u hw mgr ctx %d",
			buf_done_event_data.num_handles,ife_ctx->ctx_index,ife_hw_mgr_ctx->ctx_idx);
	...
	struct cam_isp_hw_event_info        *event_info)
{
	...
	CAM_DBG(CAM_ISP, "ctx %p Handle IFE[%u] %s event in conc ctx: %u hw mgr ctx %d",
		ctx, event_info->hw_idx,
		cam_isp_hw_evt_type_to_string(evt_id),
		ctx->concr_ctx->ctx_index,ctx->ctx_idx);
	...
	struct cam_isp_hw_event_info    *event_info)
{
	...
	CAM_DBG(CAM_ISP, "ctx %p Handle CSID[%u] %s event in conc ctx: %u hw mgr ctx %d",
		ctx, event_info->hw_idx,
		cam_isp_hw_evt_type_to_string(evt_id),
		ctx->concr_ctx->ctx_index,ctx->ctx_idx);
	...
	struct cam_isp_hw_event_info    *event_info)
{
	...
	CAM_DBG(CAM_ISP, "Handle SFE[%u] %s event in concr ctx: %u ife hw mgt ctx: %d ",
		event_info->hw_idx,
		cam_isp_hw_evt_type_to_string(evt_id),
		c_ctx->ctx_index,ctx->ctx_idx);

	switch (evt_id) {
	case CAM_ISP_HW_EVENT_ERROR:
		rc = cam_ife_hw_mgr_handle_sfe_hw_error(c_ctx, event_info);
		break;
	...
	void                                *evt_info)
{
	...
	if (rc)
		CAM_ERR(CAM_ISP, "Failed to handle %s [%u] event from hw %u in ctx %u rc %d",
			cam_isp_hw_evt_type_to_string(evt_id),
			evt_id, event_info->hw_type, c_ctx->ctx_index, rc);
	...
	void                                *evt_info)
{
	...
	CAM_DBG(CAM_ISP, "get hw mgr ctx %p ctx id %d evt_id %d ctx_idx_rd %d idx %d",
		hw_mgr_ctx, hw_mgr_ctx->ctx_idx, evt_id, ctx_idx_rd, idx);
	...
	list_for_each_entry(ife_ctx, &ife_hw_mgr->used_ctx_list, list) {
		...
		if (!ife_ctx->is_offline ||
			...
			CAM_DBG(CAM_ISP, " #REJECT# FAILED to get ife, current concr_ctx : ctx id %p : %d offline %d acquired_hw_id %d state %d",
				ife_ctx,ife_ctx->ctx_index, ife_ctx->is_offline,ife_ctx->acquired_hw_id,state);
			continue;
		}
		...
		list_for_each_entry_safe(c_elem, c_elem_temp,
			...
			if (ife_ctx->waiting_start &&
				...
				CAM_DBG(CAM_ISP,"#REJECT#: WAITING start %d input queue elem ctx idx %d start cxt idx req id %llu",
					ife_ctx->waiting_start,c_elem->ctx_idx, ife_ctx->start_ctx_idx,c_elem->request_id );
				continue;
			}
			if (!c_elem->ready) {
...
static int cam_ife_mgr_v_acquire(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	if (!acquire_hw_info) {
		CAM_ERR(CAM_ISP, "hw acquire info is NULL");
		return -EINVAL;
	}
	...
	CAM_INFO(CAM_ISP, "is IFE %d Offline %d ctx id %d", is_fe, is_offline,ife_mgr_ctx->ctx_idx);

	/* fill the HW event call back ops, to notify ISP context about the IFE hw envent, such as SOF/EOF.*/
	...
	if (is_offline) {
		 ...
		 * if current offline IFE bandwidth is not enough for required hardware bandwidth,
		 * need to acquiremore ife instance.
		 */
		if (ctx_idx <
		...
		CAM_DBG(CAM_ISP, "total IFEs used %d ife instances for offline",
				atomic_read(&ife_hw_mgr->num_acquired_offline_ctx));
		if (allocated) {
			 ...
			 * acquired hardware instance's struct as the other instance's
			 * struct.
			 */
		...
		ife_mgr_ctx->is_offline = false;
		ife_mgr_ctx->concr_ctx->flags.is_offline = false;
...
static int cam_ife_mgr_v_start_hw(void *hw_mgr_priv, void *start_hw_args)
{
	...
	if (hw_mgr_ctx->is_offline) {
		...
		for (i = 0; i < num_ctx; i++) {
			...
			/* if need more offline ife concrete, assgin one stopped concrete context.
			 * the needed number of offline ife depends on required data process bandwidth.
			 */
		...
		/* All acquired offline hw concrete contexts are started, no more hw concrete context
		 *  need to start,do nothing - no op.
		 */
		if (i == num_ctx) {
			mutex_unlock(&ife_hw_mgr->ctx_mutex);
			return 0;
